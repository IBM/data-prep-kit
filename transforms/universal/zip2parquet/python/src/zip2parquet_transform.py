...
code_data_key = "code_data"
code_data_cli_key = f"{cli_prefix}{code_data_key}"
code_data_default = True

programming_language_column_key = "programming_language_column"
programming_language_column_cli_key = f"{cli_prefix}{programming_language_column_key}"
programming_language_column_default = "programming_language"
...
class CodeToParquetTransform(AbstractBinaryTransform):
    def __init__(self, config: dict):
        ...
        self.programming_language_column = config.get(programming_language_column_key,
                                                      programming_language_column_default)
        self.treat_as_code = config.get(code_data_key, code_data_default)
                    ...
                    with opened_zip.open(member) as file:
                        try:
                            ...
                            if content_string and len(content_string) > 0:
                                ...
                                # extra processing for code
                                if self.treat_as_code and self.detect_programming_lang:
                                    lang = self._get_lang_from_ext(ext)
                                    if lang is not None:
                                        row_data[self.programming_language_column] = lang
...
class CodeToParquetTransformConfiguration(TransformConfiguration):
    ...
    def add_input_params(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            default=code_data_default,
            help="flag to process files as code"
        )
        parser.add_argument(
            f"--{programming_language_column_cli_key}",
            type=str,
            default=programming_language_column_default,
            help="Path to file containing the list of supported languages",
        )
        parser.add_argument(
            f"--{supported_langs_file_cli_key}",
            type=str,
            default=None,
            help="Path to file containing the list of supported languages",
        )
        parser.add_argument(
            f"--{detect_programming_lang_cli_key}",
            type=lambda x: bool(str2bool(x)),
