...
from dataset_stats_profiler import DatasetStatsProfiler
...
with open(data_profiler_params, "r") as json_file:
    dataset_stats_profiler_params = json.load(json_file)

# Print the loaded dictionary
print("Loaded dictionary:", dataset_stats_profiler_params)

if __name__ == "__main__":
    print("Dataset stats collection started")
    # Here we show how to run outside of the runtime
    # Create and configure the transform.
    profiler = DatasetStatsProfiler(dataset_stats_profiler_params)
    # Use the local data access to read a parquet table.
    data_access = DataAccessLocal()
    table, other_val = data_access.get_table(os.path.join(input_folder, dataset_stats_profiler_params.get("input")))
    ...
    # Update the metadata
    existing_data["DatasetStats"] = metadata
