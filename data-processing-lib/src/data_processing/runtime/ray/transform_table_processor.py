...
class TransformTableProcessorRay:
    ...
    def process_data(self, f_name: str) -> None:
        ...
        try:
            if table.num_rows > 0:
                ...
                logger.debug(f"table: {f_name} is empty, skipping processing")
    ...
    def _submit_table(self, t_start: float, out_tables: list[pa.Table], stats: dict[str, Any]) -> None:
        ...
        :return: None
        """
        logger.debug(f"submitting tables under file named {self.last_file_name}, number of tables {len(out_tables)}")
            ...
            case _:
                ...
                for index in range(count):
                    ...
                    if TransformUtils.verify_no_duplicate_columns(table=out_tables[index], file=output_name_indexed):
                        ...
                        logger.debug(
                            f"Writing transformed file {self.last_file_name}.parquet, "
                            f"{index + 1} of {count}  to {output_name_indexed}"
                        )
